...
| 0.5 | 2026-08-30 | merano（協助草擬：Claude） | 複查 prototype 程式碼近期變動，逐條核對本文件與 [SPEC.md](../SPEC.md)／[FLOW.md](../FLOW.md) 的行為描述；標註 3 處原寫成「既有行為」但目前 prototype 尚未實作的規則（主揪姓名必填、投票需逐格勾選不提供預設值、活動取消後立即擋留言），其餘內容核對後與現況相符 |
| 0.6 | 2026-08-30 | merano（協助草擬：Claude） | 文件改放到獨立的 `PRD` repo 後再次複查：把 0.5 版標註 ⚠️ 的 3 處規則直接改寫成如實描述現況（不再只加註記），並新發現多項未實作的驗證（欄位長度上限、Email 格式檢查、截止時間早於現在未做二次驗證），一併改寫並移到 SPEC.md 開放問題；同時把跨 repo 的相對路徑連結改成完整 GitHub URL，避免連結失效 |
